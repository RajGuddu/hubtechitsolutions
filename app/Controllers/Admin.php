...
class Admin extends BaseController
{
        ...
        if($this->request->getMethod() == 'post'){
            ...
            if(isset($data) && !empty($data)){
                if(!$id){
                    ...
                    $course_code_data['course_code'] = str_pad($inserted,2,'0',STR_PAD_LEFT);
                    $this->commonmodel->updateRecord('tbl_courses', $course_code_data, ['course_id '=>$inserted]);
                }else{
                    $data['update_at'] = date('Y-m-d H:i:s');
    ...
    public function admissions(){
        $url = base_url('admin/admission_list');
        if(isset($_GET['s']) || isset($_GET['course']) || isset($_GET['batch'])){
			$s = (isset($_GET['s']))?$_GET['s']:'';
			$course = (isset($_GET['course']))?$_GET['course']:'';
			$batch = (isset($_GET['batch']))?$_GET['batch']:'';
			$status = (isset($_GET['status']))?$_GET['status']:'';
            $http_query = http_build_query(array('s'=>$s,'course'=>$course,'batch'=>$batch,'status'=>$status));
            $url = base_url('admin/admission_list?'.$http_query);
			session()->set('adm_list_url', $url, 300);
		}elseif(session()->has('adm_list_url')){
			$url = session('adm_list_url');
		}
        return redirect()->to($url);
    }
    public function admission_list(){
        $this->data['courses'] = $this->commonmodel->getAllRecord('tbl_courses');
        $this->data['batches'] = $this->commonmodel->getAllRecord('tbl_batch');
        $this->data['listing'] = $this->adminmodel->get_admissions_list();
        return view('admin/ins_manage/admissions', $this->data);
    }
    public function adm_list_url_reset(){
		if(session()->has('adm_list_url')){
			session()->remove('adm_list_url');
		}
		return redirect()->to(base_url('admin/admissions'));
	}
    public function admissions_cu($id=false){
        if ($this->request->getMethod() == 'post'){
            $validation = $this->validate([
                'stu_name'=>[
                    'rules' => 'required',
                    'errors' => ['required'=>'Please enter student name!']
                ],
                'email'=>'required|valid_email',
                'course_id'=>[
                    'rules' => 'required',
                    'errors' => ['required'=>'Please select course!','valid_email'=>'Please enter valid email.']
                ],
                'batch_id'=>[
                    'rules' => 'required',
                    'errors' => ['required'=>'Please select Batch!']
                ],
            ]);
            if(!$validation){
                $this->data['validation'] = $this->validator;
                ...
                if($_FILES['stu_image']['name'] != ''){
                    if($img = $this->request->getFile('stu_image')){ 
                        ...
                        if($img->isValid() && !$img->hasMoved()){
                            $ext = explode('.',$imgname);
                            $ext = end($ext);
                            $newName = 'dp_stu_'.time().'.'.$ext;
                            $img->move('./public/assets/upload/images/',$newName);
                        }
                    }
                    $data['stu_image'] = $newName;
                }
                $data['adm_date']       = date('Y-m-d',strtotime($_POST['adm_date']));
                ...
                $data['p_address']      = $_POST['p_address'];
                if(isset($_POST['study_meterial']) && !empty($_POST['study_meterial'])){
                    $data['study_meterial'] = implode(',',$_POST['study_meterial']);
                }
                $data['other']          = $_POST['other'];
                $data['status']         = $_POST['status'];
                $data['adm_by']         = session('name');
                if(!$id){
                    $data['added_at'] = date('Y-m-d');
                    $inserted = $this->commonmodel->insertRecord('tbl_admissions', $data);
                }else{
                    $data['update_at'] = date('Y-m-d');
                    $updated = $this->commonmodel->updateRecord('tbl_admissions', $data, ['id'=>$id]);
                }
                ...
                return redirect()->to(site_url('admin/admissions'));
                
            }

        }
        if($id){
            $this->data['adm_data'] = $this->commonmodel->getOneRecord('tbl_admissions', ['id'=>$id]);
        }
        $this->data['courses'] = $this->commonmodel->getAllRecord('tbl_courses', ['status'=>1]);
        $this->data['qualification'] = $this->commonmodel->getAllRecord('tbl_qualification', ['status'=>'1']);
        $this->data['batches'] = $this->commonmodel->getAllRecordOrderByDesc('tbl_batch','',['batch_id','desc']);
        return view('admin/ins_manage/admissions_cu', $this->data);
    }
    public function admissions_r($id=false){
        $this->data['student'] = $this->adminmodel->get_admissions_list(['a.id'=>$id]);
        return view('admin/ins_manage/admissions_r', $this->data);
    }
    public function admission_d($id=false){
        if(!$id){
            return redirect()->to(site_url('admin/admission_list'));
        }else{
            $deleted = $this->commonmodel->deleteRecord('tbl_admissions',['id'=>$id]);
            if($deleted){
                $this->session->setFlashdata(['message'=>'Record Deleted Successfully', 'type'=>'success']);
            }else{
                $this->session->setFlashdata(['message'=>'Record Not Delete. Please try again...', 'type'=>'danger']);
            }
            return redirect()->to(base_url('/admin/admission_list'));
        }
    }
    public function batches(){
        $this->data['batches'] = $this->commonmodel->getAllRecordOrderByDesc('tbl_batch','',['batch_id','desc']);
        return view('admin/ins_manage/batch_index', $this->data);
    }
    public function batch_cu($id = false){
        if ($this->request->getMethod() == 'post'){
            $validation = $this->validate([
                'batch_name'=>[
                    'rules' => 'required',
                    'errors' => ['required'=>'Batch Name is required!']
                ],
                'date_from' => 'required',
                'date_to' => 'required',
                'time_from' => 'required',
            ...
            if(!$validation){
                ...
                if(!$id){
                    $data['added_by']       = session('name');
                    $data['added_at']       = date('Y-m-d');
                    $inserted = $this->commonmodel->insertRecord('tbl_batch', $data);
                }else{
                    $data['update_by']      = session('name');
                    $data['update_at']      = date('Y-m-d');
                    $updated = $this->commonmodel->updateRecord('tbl_batch', $data, ['batch_id'=>$id]);
                }
                ...
                return redirect()->to(site_url('admin/batches'));
                
            }

        }
        if($id){
            $this->data['batch'] = $this->commonmodel->getOneRecord('tbl_batch', ['batch_id'=>$id]);
        }
        //$this->data['courses'] = $this->commonmodel->getAllRecord('tbl_courses', ['status'=>1]);
        //$this->data['qualification'] = $this->commonmodel->getAllRecord('tbl_qualification', ['status'=>'1']);
        //$this->data['batches'] = $this->commonmodel->getAllRecord('tbl_batch', ['status'=>1]);
        return view('admin/ins_manage/batch_cu', $this->data);
    }
    public function centers(){
        $this->data['centers'] = $this->commonmodel->getAllRecordOrderByDesc('tbl_franchises','',['fra_id','desc']);
        return view('admin/franchise/franchise_index', $this->data);
    }
    public function center_cu($id=false){
        if($this->request->getMethod() == 'post'){
            $validation = $this->validate([
            ...
            if(!$validation){
                ...
                $data['center_code']    = $_POST['center_code'];
                $data['city']           = $_POST['city'];
                $data['director_name']  = $_POST['director_name'];
                $data['status']         = $_POST['status'];
                
                if(!$id){
                    $data['added_by']       = session('name');
                    $data['added_at']       = date('Y-m-d');
                    $inserted = $this->commonmodel->insertRecord('tbl_franchises', $data);
                }else{
                    $data['update_by']      = session('name');
                    $data['updated_at']      = date('Y-m-d');
                    $updated = $this->commonmodel->updateRecord('tbl_franchises', $data, ['fra_id'=>$id]);
                }
                ...
                return redirect()->to(site_url('admin/centers'));
                
            }

        }
        if($id){
            $this->data['center'] = $this->commonmodel->getOneRecord('tbl_franchises', ['fra_id'=>$id]);
        }
        //$this->data['courses'] = $this->commonmodel->getAllRecord('tbl_courses', ['status'=>1]);
        //$this->data['qualification'] = $this->commonmodel->getAllRecord('tbl_qualification', ['status'=>'1']);
        //$this->data['batches'] = $this->commonmodel->getAllRecord('tbl_batch', ['status'=>1]);
        return view('admin/franchise/franchise_cu', $this->data);
    }
    public function certificate_list(){
        $this->data['cartList'] = $this->commonmodel->getAllRecordOrderByDesc('tbl_certificate_list','',['sl_no','desc']);
        return view('admin/ins_manage/certificate_list', $this->data);
    }
    public function certificate_cu($id=false){
        if($this->request->getMethod() == 'post'){
            $validation = $this->validate([
                //'cert_no'=>'required',
                ...
                'cert_no'=>[
                    'rules' => 'required',
                    'errors' => ['required'=>'Please enter Certificate Number.',]
                ],
                'enrollment_no'=>[
                    'rules' => 'required',
                    'errors' => ['required'=>'Please enter Enrollment Number.']
                ],
                'student_name'=>[
                    'rules' => 'required',
                    'errors' => ['required'=>'Please enter Student\'s Name.']
                ],
                'f_name'=>[
                    'rules' => 'required',
                    'errors' => ['required'=>'Please enter Father\'s Name.']
                ],
                'course'=>[
                    'rules' => 'required',
                    'errors' => ['required'=>'Please enter Course.']
                ],
            ]);
            if(!$validation){
                $this->data['validation'] = $this->validator;
                ...
                $data['cert_no']    = $_POST['cert_no'];
                $data['enrollment_no']    = $_POST['enrollment_no'];
                ...
                if(!$id){
                    //$data['added_by']       = session('name');
                    //$data['added_at']       = date('Y-m-d');
                    $inserted = $this->commonmodel->insertRecord('tbl_certificate_list', $data);
                }else{
                    // $data['update_by']      = session('name');
                    // $data['updated_at']      = date('Y-m-d');
                    $updated = $this->commonmodel->updateRecord('tbl_certificate_list', $data, ['sl_no'=>$id]);
                }
                ...
                return redirect()->to(site_url('admin/certificate_list'));
                
            }

        }
        if($id){
            $this->data['cert'] = $this->commonmodel->getOneRecord('tbl_certificate_list', ['sl_no'=>$id]);
        }
        //$this->data['courses'] = $this->commonmodel->getAllRecord('tbl_courses', ['status'=>1]);
        //$this->data['qualification'] = $this->commonmodel->getAllRecord('tbl_qualification', ['status'=>'1']);
        //$this->data['batches'] = $this->commonmodel->getAllRecord('tbl_batch', ['status'=>1]);
        return view('admin/ins_manage/certificate_cu', $this->data);
    }
    public function certificate_d($id=false){
        if(!$id){
            return redirect()->to(site_url('admin/certificate_list'));
        }else{
            $deleted = $this->commonmodel->deleteRecord('tbl_certificate_list',['sl_no'=>$id]);
            if($deleted){
                $this->session->setFlashdata(['message'=>'Record Deleted Successfully', 'type'=>'success']);
            }else{
                $this->session->setFlashdata(['message'=>'Record Not Delete. Please try again...', 'type'=>'danger']);
            }
            return redirect()->to(base_url('/admin/certificate_list'));
        }
    }
}
?>